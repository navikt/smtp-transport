...
class EmailMsgFilterSpec : StringSpec({
    ...
    "filterMessageForwarding returns configured system when service CPA id is in the list" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "all", whitelist = TEST_CPA_IDS_FILE))
    ...
    "filterMessageForwarding matches CPA ids case-insensitively" {
        val rules = rulesOf(ServiceFilter(name = "urn:oasis:names:tc:ebxml-msg:service", both = true, selection = "all", whitelist = TEST_CPA_IDS_FILE))
    ...
    "filterMessageForwarding returns EMOTTAK when service CPA id is not in the list" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "lastDigit9", whitelist = TEST_CPA_IDS_FILE))
    ...
    "filterMessageForwarding returns configured system for any CPA id when service is configured with all" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "all"))
    ...
    "filterMessageForwarding matches service names case-sensitively" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL.uppercase(), both = true, selection = "all"))
    ...
    "filterMessageForwarding keeps the original message when forwarding to EMOTTAK" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "none", whitelist = TEST_CPA_IDS_FILE))
    ...
    "building rules fails when the CPA id file is missing" {
        val exception = shouldThrow<IllegalStateException> {
            rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "all", whitelist = "cpa/test/does-not-exist.txt"))
        }
    ...
    "building rules fails when selection is invalid" {
        val exception = shouldThrow<IllegalStateException> {
            rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "firstdigit1"))
        }
    ...
    "building rules fails when percentage is invalid" {
        val exception = shouldThrow<NumberFormatException> {
            rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "percentageXX"))
        }
    ...
    "building rules fails when last digit setting is invalid" {
        val exception = shouldThrow<IllegalArgumentException> {
            rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "lastDigitXX"))
        }
    ...
    "building rules fails on duplicate service names" {
        val exception = shouldThrow<IllegalStateException> {
            rulesOf(
                ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "all"),
                ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "all")
            )
        }
 ...
 */
    ...
    "filterMessageForwarding returns EMOTTAK when service is NOT configured" {
        val rules = rulesOf(ServiceFilter(name = "someOtherService", both = true, selection = "all"))
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EMOTTAK
    }

    "filterMessageForwarding returns EMOTTAK when service is configured with NONE + BOTH/SPLIT" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "none"))
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EMOTTAK
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "none"))
    ...
    "filterMessageForwarding returns EBMS/BOTH when service is configured with ALL + SPLIT/BOTH" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "all"))
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EBMS
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "all"))
    ...
    "filterMessageForwarding returns EMOTTAK when service is configured with UNmatching CPA-ID lastdigit + BOTH/SPLIT" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "lastDigit234567890")) // 1 would match
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EMOTTAK
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "lastDigit234567890"))
        PAYLOAD_MESSAGE.forwardingSystem(rules2) shouldBe ForwardingSystem.EMOTTAK
    }

    "filterMessageForwarding returns EBMS/BOTH when service is configured with matching CPA-ID lastdigit + SPLIT/BOTH" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "lastDigit1"))
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EBMS
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "lastDigit1"))
        PAYLOAD_MESSAGE.forwardingSystem(rules2) shouldBe ForwardingSystem.BOTH
    }

    "filterMessageForwarding returns EMOTTAK when service is configured with UNmatching percentage + BOTH/SPLIT" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "percentage0")) // 0 percent will be routed
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EMOTTAK
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "percentage0"))
        PAYLOAD_MESSAGE.forwardingSystem(rules2) shouldBe ForwardingSystem.EMOTTAK
    }

    "filterMessageForwarding returns EBMS/BOTH when service is configured with matching percentage + SPLIT/BOTH" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "percentage101")) // 100 percent will be routed
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EBMS
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "percentage101"))
        PAYLOAD_MESSAGE.forwardingSystem(rules2) shouldBe ForwardingSystem.BOTH
    }
    ...
    "filterMessageForwarding returns EBMS/BOTH in all cases that normally give EMOTTAK when whitelist includes the CPA-ID" {
        ...
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "none", whitelist = TEST_CPA_IDS_FILE))
        PAYLOAD_MESSAGE.forwardingSystem(rules2) shouldBe ForwardingSystem.EBMS
        val rules3 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "lastDigit234567890", whitelist = TEST_CPA_IDS_FILE)) // 1 would match
    ...
    "filterMessageForwarding returns EMOTTAK in all cases that normally give EBMS/BOTH when blacklist includes the CPA-ID" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "all", blacklist = TEST_CPA_IDS_FILE))
        PAYLOAD_MESSAGE.forwardingSystem(rules) shouldBe ForwardingSystem.EMOTTAK
        val rules2 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "all", blacklist = TEST_CPA_IDS_FILE))
        PAYLOAD_MESSAGE.forwardingSystem(rules2) shouldBe ForwardingSystem.EMOTTAK
        val rules3 = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = false, selection = "lastDigit1", blacklist = TEST_CPA_IDS_FILE))
    ...
    "resolveForwarding reports BOTH when last digit in CPA-ID matches, EMOTTAK otherwise" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "lastDigit24680"))
    ...
    "resolveForwarding reports BOTH when percentage setting gets hit, EMOTTAK otherwise" {
        val rules = rulesOf(ServiceFilter(name = INNTEKTSFORESPORSEL, both = true, selection = "percentage50"))
        var count_both = 0
        var count_emottak = 0
        for (i in 1..1000) {
